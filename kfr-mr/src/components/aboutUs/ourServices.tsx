...
const ValuesSection = ({ backgroundImage }: ValuesSectionProps) => {
  ...
  const textBlur = (1 - scrollProgress) * 10; // Blur from 10px to 0px
  ...
  return (
    <div className="min-h-screen relative overflow-hidden mb-12">
      ...
      <div className={`absolute inset-0 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 ${backgroundImage ? 'opacity-50' : 'opacity-100'}`}></div>
      ...
      <div className="relative z-10 container mx-auto px-6 py-20">
        {/* Header Text with Scroll Animation */}
...
const ValueCardComponent = ({ card, delay }: ValueCardProps) => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    // Prevent any click functionality on mobile
    if (isMobile) {
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  return (
    <div
      className={`group relative transition-all sm:pt-20 duration-500 ease-out ${!isMobile ? 'hover:scale-105' : ''}`}
    ...
    >
      ...
      >
        ...
        {/* Extended content area that appears on hover */}
        <div 
          className={`
          ...
          `}
          style={{
            background: isHovered && !isMobile ? `linear-gradient(135deg, ${card.gradient.replace('from-', '').replace(' to-', ', ')})` : 'transparent'
          }}
        />
        
        {/* Content Container - positioned to stay at bottom */}
